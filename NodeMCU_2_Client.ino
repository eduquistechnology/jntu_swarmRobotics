...
IPAddress remoteIP(192, 168, 4, 1); // replace with the IP address of the other NodeMCU
...
void loop() {
  ...
  udp.write("Hello, NodeMCU 1!");
  udp.endPacket();
  Serial.println("Sent data to NodeMCU 1.");
  ...
  if (packetSize) {
    ...
    if (len > 0) {
      packetBuffer[len] = '\0';
      Serial.print("Received data from NodeMCU 1: ");
